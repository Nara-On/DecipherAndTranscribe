...
class CharacterER(BaseMetric):
    """Characcter Error Rate metric."""
    ...
    def __call__(
        ...
        for model_out, gt, ln in zip(output, batch.gt, batch.gt_len):
            ...
            #predsStr = self.decodePred(preds)
            predsStr = self.vocab.decode(preds)
            targetStr = self.vocab.decode(target)
            
            cer = CharErrorRate()
            out.append({self.METRIC_NAME: str(cer(" ".join(predsStr), " ".join(targetStr)).item())})
        return out
    
    
    def decodePred(self, encoded: List[int]) -> List[str]:
        """Convert the input token sequence back to a list of textual tokens, 
            when tokens may not be found in list of textual tokens, 
        ...
        decoded = []
        for x in encoded:
            if x < len(self.vocab.index2vocab):
                decoded.append(self.vocab.index2vocab[x])
            else:
