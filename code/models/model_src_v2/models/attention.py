...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, hidden, encoder_output, enc_len, prev_attention):
        ...
        attn_weight = Variable(torch.zeros(attn_energy.shape)).to(self.device)
    ...
    def score(self, hidden, encoder_output):
        ...
        addMask = torch.FloatTensor([1 / self.decoder_layer] * self.decoder_layer).view(
            1, self.decoder_layer, 1
        )
        addMask = torch.cat([addMask] * hidden.shape[0], dim=0)
        addMask = Variable(addMask.to(self.device))  # batch, layers, 1
...
class locationAttention(nn.Module):
    ...
    def score(self, hidden, encoder_output, prev_attention):
        ...
        addMask = torch.cat([addMask] * hidden.shape[0], dim=0)
        addMask = Variable(addMask.to(self.device))  # b, layers, 1
