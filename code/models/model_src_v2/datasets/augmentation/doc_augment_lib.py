...
def GammaCorrection(img, gamma=(0.3, 3.0), clip=(0.0, 1.0)):
...
def PixelPosition(img, sigma=5.0, order=1, maxdelta=5.0):
    ...
    xy = np.transpose(
        np.array(np.meshgrid(list(range(H)), list(range(W)))), axes=[0, 2, 1]
    )
    deltas += xy
...
def ElasticTransform(img, alpha=1750, sigma=45):
    ...
    transformed_image = ndi.map_coordinates(
        img, distorted_indices, mode="nearest", order=1
    ).reshape(image_shape)
...
def LensBlurThr(img, lens_blur=(0.0, 2.0)):
    ...
    blurred = ndi.gaussian_filter(img, lens_blur)
    _, bw = cv2.threshold(
        np.uint8(img * 255), 0, 1, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )
...
def Sharpen(img, lens_blur=(0.0, 2.0)):
    ...
    res = cv2.addWeighted(img, 1.5, blurred, -0.5, 0)
    return res
...
def RandomBackground(
    img,
    scales=[1.0, 5.0, 10.0, 20.0, 50.0, 100.0],
    ...
    result = (
        ndi.zoom(
            np.float32(np.random.rand(int(h / scales[0] + 1), int(w / scales[0] + 1))),
            scales[0],
        )[:h, :w]
        * weights[0]
    )
    for we, s in zip(scales[1:], weights[1:]):
        result += (
            ndi.zoom(np.random.rand(int(h / s + 1), int(w / s + 1)), s)[:h, :w] * we
        )
...
def Shear(img, shear=(-0.5, 0.25), background=0):
...
def ShearNoPad(img, shear=(-0.5, 0.25)):
...
def PerspectiveNoPad(img, step_x=20):
    ...
    B = np.array(
        [
            [
                step_x + np.random.randint(-step_x, step_x),
                step_x + np.random.randint(-step_y, step_y),
            ],
            [
                step_x + w + np.random.randint(-step_x, step_x),
                step_x + np.random.randint(-step_y, step_y),
            ],
            [
                step_x + w + np.random.randint(-step_x, step_x),
                step_x + h + np.random.randint(-step_y, step_y),
            ],
            [
                step_x + np.random.randint(-step_x, step_x),
                step_x + h + np.random.randint(-step_y, step_y),
...
def KanungoNoise(img, alpha=2.0, beta=2.0, alpha_0=1.0, beta_0=1.0, mu=0.05, k=2):
    ...
    closing = cv2.morphologyEx(
        distorted, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
    )
    return closing * 255
