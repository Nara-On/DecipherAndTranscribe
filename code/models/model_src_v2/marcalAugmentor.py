...
def augmentor(img):
    ...
    param_kanungo_alpha = (
        2  # params controlling how much foreground and background pixels flip state
    )
    ...
    param_min_shear = -0.5  # here a little bit more shear to the left than to the right
    param_max_shear = 0.25

    param_rotation = 3  # plus minus angles for rotation

    param_scale = 0.2  # one plus minus parameter as scaling factor

    param_movement_BB = 6  # translation for cropping errors in pixels

    # add gaussian noise
    gauss = np.random.normal(
        param_mean_gaussian_noise, param_sigma_gaussian_noise, (TH, TW)
    )
    ...
    otsu_th, binarized = cv2.threshold(
        gammacorrected, 0, 1, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )

    # Kanungo noise
    dist = cv2.distanceTransform(
        1 - binarized, cv2.DIST_L1, 3
    )  # try cv2.DIST_L1 for newer versions of OpenCV
    dist2 = cv2.distanceTransform(
        binarized, cv2.DIST_L1, 3
    )  # try cv2.DIST_L1 for newer versions of OpenCV
    ...
    P = (
        param_kanungo_alpha0 * np.exp(-param_kanungo_alpha * dist ** 2)
    ) + param_kanungo_mu
    P2 = (
        param_kanungo_beta0 * np.exp(-param_kanungo_beta * dist2 ** 2)
    ) + param_kanungo_mu
    ...
    closing = cv2.morphologyEx(
        distorted,
        cv2.MORPH_CLOSE,
        np.ones((param_kanungo_k, param_kanungo_k), dtype=np.uint8),
    )
    ...
    while len(points) < 1:
        ...
        sheared = cv2.warpAffine(
            canvas, M, (3 * TW, 3 * TH), flags=cv2.WARP_INVERSE_MAP | cv2.INTER_CUBIC
        )

        # random rotation
        M = cv2.getRotationMatrix2D(
            (3 * TW / 2, 3 * TH / 2),
            np.random.uniform(-param_rotation, param_rotation),
            1,
        )
        rotated = cv2.warpAffine(
            sheared, M, (3 * TW, 3 * TH), flags=cv2.WARP_INVERSE_MAP | cv2.INTER_CUBIC
        )
...
if __name__ == "__main__":
    ...
    out_imgs = [
        cv2.resize(
            augmentor(img), (img.shape[1], img.shape[0]), interpolation=cv2.INTER_AREA
        )
        for i in range(20)
    ]
